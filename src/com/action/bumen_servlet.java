...
public class bumen_servlet extends HttpServlet
{
	...
	public void bumenDel(HttpServletRequest req,HttpServletResponse res)
	{
		...
		String sql1="delete from t_gongzi ta where ta.zhigong_id in (SELECT tc.id FROM t_bumen tb,t_zhigong tc  WHERE tb.del = 'yes' and tb.id = tc.bumen_id) ";
		Object[] params={};
		DB mydb=new DB();
		mydb.doPstm(sql, params);
	...
	{
		String sql="select * from t_bumen where del='no' AND mingcheng != '财务部'";
