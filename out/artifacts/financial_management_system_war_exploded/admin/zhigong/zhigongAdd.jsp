...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
        ...
        <script language="javascript">
            ...
            function openBumen(path)
            {
				var url=path+"/bumen?type=bumenSele";
       	   		window.open(url,"部门信息选择","dialogWidth=500px;dialogHeight=300px;status=yes;help=no;scroll=yes");
				window.addEventListener('storage', function(event) {
					if (event.key === 'sharedData') {
						document.getElementById('bumen_id').value = event.newValue[0];

						document.getElementById('bmmc').value = event.newValue.substring(2);
					}
				});
			...
			<form action="<%=path %>/zhigong?type=zhigongAdd" name="formAdd" method="post">
				     <table width="98%" align="center" border="0" cellpadding="4" cellspacing="1" bgcolor="#CBD8AC" style="margin-bottom:8px">
						...
						<tr align='center' bgcolor="#FFFFFF" height="22">
						    ...
						    <td width="75%" bgcolor="#FFFFFF" align="left">
								<input type="text" id="bmmc"  onclick="openBumen('<%=path %>')" readonly />点击选择部门
